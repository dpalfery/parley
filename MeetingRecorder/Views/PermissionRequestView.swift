...
struct PermissionRequestView: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 40)
        .alert("Permission Required", isPresented: $appEnvironment.permissionManager.showPermissionAlert) {
            Button("Open Settings") {
                appEnvironment.permissionManager.openSettings()
            }
            Button("Cancel", role: .cancel) {
                isPresented = false
            }
        } message: {
            Text(appEnvironment.permissionManager.permissionAlertMessage)
        }
    }
