...
final class SpeakerServiceTests: XCTestCase {
    ...
    // MARK: - Speaker Segment Overlap Tests
    
    func testSpeakerSegmentContainsTimestamp() {
        // Given: A speaker segment
        let segment = SpeakerSegment(speakerID: "speaker-1", startTime: 5.0, endTime: 10.0)
        
        // When: Checking various timestamps
        // Then: Should correctly identify contained timestamps
        XCTAssertTrue(segment.contains(timestamp: 5.0))
        XCTAssertTrue(segment.contains(timestamp: 7.5))
        XCTAssertTrue(segment.contains(timestamp: 10.0))
        XCTAssertFalse(segment.contains(timestamp: 4.9))
        XCTAssertFalse(segment.contains(timestamp: 10.1))
    }
    
    func testSpeakerSegmentFormattedTimeRange() {
        // Given: A speaker segment
        let segment = SpeakerSegment(speakerID: "speaker-1", startTime: 65.0, endTime: 125.0)
        
        // When: Getting formatted time range
        let formatted = segment.formattedTimeRange
        
        // Then: Should be properly formatted (MM:SS - MM:SS)
        XCTAssertEqual(formatted, "01:05 - 02:05")
    }
    
    // MARK: - Speaker Profile Helper Methods Tests
    
    func testSpeakerProfileWithUpdatedName() {
        // Given: A speaker profile
        let profile = SpeakerProfile(
            id: "speaker-1",
        ...
        // When: Creating updated profile with new name
        let updated = profile.withUpdatedName("New Name")
        
        // Then: Should have new name but same ID
        XCTAssertEqual(updated.id, profile.id)
        XCTAssertEqual(updated.displayName, "New Name")
        XCTAssertGreaterThanOrEqual(updated.lastUsed, profile.lastUsed)
    }
    
    func testSpeakerProfileWithUpdatedLastUsed() {
        // Given: A speaker profile
        ...
        // When: Creating updated profile with new last used
        let updated = profile.withUpdatedLastUsed()
        
        // Then: Should have updated last used timestamp
        XCTAssertEqual(updated.id, profile.id)
        XCTAssertEqual(updated.displayName, profile.displayName)
        XCTAssertGreaterThan(updated.lastUsed, profile.lastUsed)
    }
}
